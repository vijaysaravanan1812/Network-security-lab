...
{  
    ...
    getline(cin,msg);
    int original_len = msg.length(), padded_len=original_len;
    if (padded_len%16!=0)
    {
    ...
    bytes plain[padded_len];

    cout <<endl<<"Plaintext to be encrypted: "<<msg<<endl;  
    for(int  j=0; j<padded_len;j+=16){
        for(int i=0; i<16; ++i)  
        {
            plain[i]=bytes(msg[i]);
        }
        encrypt(plain, w);
        for(int i =0; i<16;++i)
            encrypted_msg+=plain[i].to_ulong();
    }

    cout<<"Encrypted ciphertext:"<<encrypted_msg<<endl; 
    string decrypted_msg;
    for(int  j=0; j<padded_len;j+=16){
        for(int i=0; i<16; ++i)  
        {
            plain[i]=bytes(encrypted_msg[i]);
        }
        decrypt(plain, w);
        for(int i =0; i<16;++i)
            decrypted_msg+=plain[i].to_ulong();
    }
    cout << "Decrypted plaintext: "<<decrypted_msg<<endl;
    encrypted_msg.clear();
    decrypted_msg.clear();
    return 0;  
}
